...
Logger_Dispatcher* g_svc = nullptr;
...
#define SVCCMD_FTP_UPLOAD		0x00000089  // 137
...
DWORD WINAPI Service_Ctrl(DWORD dwCtrlCode, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext)
{
	...
	case SVCCMD_FLUSH_FILE: //136
		...
		ret = NO_ERROR;
		break;
	case SVCCMD_FTP_UPLOAD: //137
		LOGTO(plogfile, Logging::LL_Info, Logging::LC_Service, "Initiating SFTP upload due to service control command");
		g_svc->enqueue<Logger_Dispatcher::evFtpUpload>();
		ret = NO_ERROR;
		break;
		...
		if (argv[x][0] == '-' || argv[x][0] == '/')
		{
			...
			for (int y = 1; y < optlen; ++y)
			{
				...
				case L'l': // specify log file
					if (y == optlen - 1 && ++x < argc && argv[x][0] != _T('-') && argv[x][0] != _T('/'))
						logfile = argv[x];
					else
					{
						std::cout << "Invalid command line parameters" << std::endl;
